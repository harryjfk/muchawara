...
use App\Plugins\LandingPagesPlugin\Models\FollowUs;
...
class LandingPageRepository {
    ...
    public static function followUsCallback()
    {
        $folowUsLinks = FollowUs::where('activated', 'yes')->get()->sortByDesc('priority');
        return Theme::view('plugin.LandingPagesPlugin.follow_us', ['follow_us_links' => $folowUsLinks]);
    }



    public static function allFollowUsLinks()
    {
        return FollowUs::all();
    }


    public static function createFollowUsLink($icon_script, $label, $hover_text, $redirect_url, $priority)
    {
        $followUs = new FollowUs;
        $followUs->icon_script = $icon_script;
        $followUs->label = $label;
        $followUs->hover_text = $hover_text;
        $followUs->redirect_url = $redirect_url;
        $followUs->priority = $priority;
        $followUs->save();
        return [
            "status" => "success",
            "success_type" => "FOLLOW_US_LINK_CREATED",
            "success_text" => trans('LandingPagesPluginAdmin.follow_us_link_created')
        ];
    }


    public static function deleteFollowUsLink($followUsID)
    {
        $followUs = FollowUs::find($followUsID);
        $followUs->forceDelete();
        return [
            "status" => "success",
            "success_type" => "FOLLOW_US_LINK_DELETED",
            "success_text" => trans('LandingPagesPluginAdmin.follow_us_link_deleted')
        ];
    }



    public static function activateFollowUsLink($id, $activate = 'yes')
    {
        $followUs = FollowUs::find($id);
        $followUs->activated = $activate;
        $followUs->touch();
        $followUs->save();
        
        return [
            "status" => "success",
            "success_type" => ($activate == 'yes') ? "FOLLOW_US_LINK_ACTIVATED" : "FOLLOW_US_LINK_DEACTIVATED",
            "success_text" => ($activate == 'yes') ? trans('LandingPagesPluginAdmin.follow_us_link_activated') : trans('LandingPagesPluginAdmin.follow_us_link_deactivated')
        ]; 
