...
class LandingPagesPlugin extends PluginAbstract {
	...
	public function hooks() {
		...
		Plugin::add_hook('composing:*', function ($view) {
			...
			if ($custom_landing != '' && $custom_landing != '-1') {
				...
				if(!(strpos($view->getName(), 'auth.registration') == false)) {
					...
					$view->with('total_signups', app('App\Repositories\Admin\DashboardRepository')->getTotalSignUps());
		...
		Theme::hook('follow_us', [App\Repositories\LandingPageRepository::class, 'followUsCallback']);
...
VIDEO_SCRIPT;
	...
	public function routes() {
		...
		Route::group(['middleware' => 'admin'], function () {
			...
			Route::post('admin/plugins/landing-pages-setting/follow-us-settings/create', 'App\Http\Controllers\LandingPagesController@createFollowUsLink');
			Route::post('admin/plugins/landing-pages-setting/follow-us-settings/delete', 'App\Http\Controllers\LandingPagesController@deleteFollowUsLink');
			Route::post('admin/plugins/landing-pages-setting/follow-us-settings/actiavte', 'App\Http\Controllers\LandingPagesController@activateFollowUsLink');
