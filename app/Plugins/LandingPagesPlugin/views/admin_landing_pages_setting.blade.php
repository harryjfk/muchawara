...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="col-md-12 section-first-col">
            ...
            <div class= "row">            
                <div class="col-md-10 add-creditpackage-col admin-create-div">
                    <p class="add-credit-package-text">{{{trans('LandingPagesPluginAdmin.follow_us_heading')}}}</p>
                    <div style="overflow-x: auto">
                        
                    <table class="table table-responsive">
                        <thead>
                          <tr>
                            <th>{{{trans('LandingPagesPluginAdmin.follow_us_icon')}}}</th>
                            <th>{{{trans('LandingPagesPluginAdmin.follow_us_text')}}}</th>
                            <th>{{{trans('LandingPagesPluginAdmin.follow_us_hover_text')}}}</th>
                            <th>{{{trans('LandingPagesPluginAdmin.follow_us_redirect_url')}}}</th>
                            <th>{{{trans('LandingPagesPluginAdmin.follow_us_priority')}}}</th>
                            <th>{{{trans('LandingPagesPluginAdmin.follow_us_activated')}}}</th>
                            <th>{{{trans('LandingPagesPluginAdmin.follow_us_action')}}}</th>
                          </tr>
                        </thead>
                        <tbody>
                            @foreach($follow_us_links as $link)
                              <tr>
                                <td>{!! $link->icon_script !!}</td>
                                <td>{{$link->label}}</td>
                                <td>{{$link->hover_text}}</td>
                                <td>{{$link->redirect_url}}</td>
                                <td>{{$link->priority}}</td>
                                <td>
                                    <label class="switch">
                                       <input class="switch-input follow-us-checkbox" data-id="{{$link->id}}" type="checkbox" @if($link->activated == 'yes') checked @endif>
                                       <span class="switch-label"></span> 
                                       <span class="switch-handle"></span> 
                                    </label>
                                </td>
                                <td>
                                    <i class="fa fa-trash-o delete-folow-us" data-id = "{{$link->id}}" title="{{trans('LandingPagesPluginAdmin.delete_follow_us')}}"></i>
                            ...
                            <tr>
                                <td colspan="" ="7"><label>{{{trans('LandingPagesPluginAdmin.create_new_follow_us_link')}}}</label></td>
                                <td></td>                                
                                <td></td>                                
                                <td></td>                                
                                <td></td>                                
                            ...
                            <tr>
                                <td><input type="text" name="icon_script" style="color:black;width:100%" placeholder="{{{trans('LandingPagesPluginAdmin.follow_us_icon')}}}"></td>
                                <td><input type="text" name="label" style="color:black;width:100%" placeholder="{{{trans('LandingPagesPluginAdmin.follow_us_text')}}}"></td>
                                <td><input type="text" name="hover_text" style="color:black;width:100%" placeholder="{{{trans('LandingPagesPluginAdmin.follow_us_hover_text')}}}"></td>
                                <td><input type="text" name="redirect_url" style="color:black;width:100%" placeholder="{{{trans('LandingPagesPluginAdmin.follow_us_redirect_url')}}}"></td>
                                <td><input type="text" name="priority" style="color:black;width:100%" placeholder="{{{trans('LandingPagesPluginAdmin.follow_us_priority')}}}"></td>
                                <td><button type="button" id="creat_follow_us" class="btn btn-info btn-addpackage btn-custom" style="float:left;margin-top: 0px">{{{trans('admin.new')}}}</button></td>
                            </tr>
                        </tbody>
                    </table>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $(".follow-us-checkbox").on("click", function(){

            var URL = "{{url('admin/plugins/landing-pages-setting/follow-us-settings/actiavte')}}";
            var data = {
                _token : "{{csrf_token()}}",
                id : $(this).data('id'),
                activate : $(this).prop('checked') ? 'yes' : 'no'
            };
        ...
        $(".delete-folow-us").on("click", function(){

            var URL = "{{url('admin/plugins/landing-pages-setting/follow-us-settings/delete')}}";
            var data = {
                _token : "{{csrf_token()}}",
                id : $(this).data('id')
            };
        ...
        $("#creat_follow_us").on("click", function(){

            var URL = "{{url('admin/plugins/landing-pages-setting/follow-us-settings/create')}}";
            var data = {
                _token : "{{csrf_token()}}",
                icon_script : $("input[name=icon_script]").val(),
                label : $("input[name=label]").val(),
                hover_text : $("input[name=hover_text]").val(),
