...
class WebsocketChatRepository {
	...
	public static function saveMessage($data) {
		...
		try {
			...
			if(isset($data['message_meta'])) {
				$message->meta = $data['message_meta'];
			}
	...
	public static function getMessages ($user_id, $contact_id, $last_message_id, &$loadMoreFlag = false) {
		...
		if ($last_message_id == 0) {
			...
			})->orderBy('created_at', 'desc')->take(21)->get();
			...
			$messages = Message::where('contact_id', $contact_id)->where(function($query)use($user_id){
				...
				$query->whereNotIn('from_user', self::$blockUserIDs)->whereNotIn('to_user', self::$blockUserIDs);
			})->where('id', '<', $last_message_id)->orderBy('created_at', 'desc')->take(21)->get();
		}
		...
		foreach($messages as $message) {
            ...
            Plugin::fire("get_messages_each", ["message" => $message]);
        }
    ...
    public static function countMessagesByDate($userId, $lastLogin) {
        $countMessages = Message::where('to_user', $userId)
                         ->where('created_at', '>', $lastLogin)->count();
			...
			if ($last_contact_id > 0) {
				...
				$contacts = ChatContact::where(function($query) use($authUser){
                    ...
                    ->where(function($query) use($authUser){
                        $query ->whereRaw("contacts.user2 IN (SELECT user2 FROM matches WHERE user1 = {$authUser->id})")
                            ->orWhereRaw("contacts.user1 IN (SELECT user1 FROM matches WHERE user2 = {$authUser->id})");

                    })

                    ->where(function($query){
					$query->whereNotIn('user1', self::$blockUserIDs)->whereNotIn('user2', self::$blockUserIDs);
				})->where('id', '<', $last_contact_id)->orderBy('created_at', 'desc')->take(200)->get();
...
//                }
	        ...
	        foreach ($users as $user) {
	            ...
	            // this line is comment by kike
//	        	$user->profile_picture = $user->thumbnail_pic_url();
	        	$user->profile_picture = $user->others_pic_url();
		    	...
		    	$fakeContact->id = 0;
		    	$fakeContact->created_at = null;

		    	if(self::canInitiateChat($user, $fakeContact, $error_type)) {
		    		$user->can_init_chat = true;
		    	} else {
		    		$user->can_init_chat = false;
		    		$user->init_chat_error_type = $error_type;
		    	}
...
//    	$contact_user->profile_picture = $contact_user->thumbnail_pic_url();
    	$contact_user->profile_picture = $contact_user->others_pic_url();
