...
<div class="modal-dialog">
                ...
                <div class="row">
                    ...
                    <div id="all-col" class="col-md-12 col-xs-12 user_main_col_height user_main_col_border_bottom  ">
                        <div class="row users_list_cnt">
                            <div  ng-click="loadChatData(user,$index)" data-index="[[$index]]" data-id="user-[[user.user.id]]" ng-class="{active: $index == selected}" class="col-md-12 col-xs-12 user_div_padding left-first-div all_users" ng-repeat="user in chatUsers |filter:userSearch">
    ...
    box-shadow:  1px 1px 6px 1px rgba(0, 0, 0, 0.52);font-size: 12px;">[[user.user.total_unread_messages_count]]</span>
                    ...
                    <!-- Favourite users content-->
                    <div id="star-col" class="col-md-12 col-xs-12 user_main_col_height user_main_col_border_bottom " style="display:none">
                        <div class="row users_list_cnt">
                            <div class="col-md-12 col-xs-12 user_div_padding">
                                <div class="col-md-4 col-xs-4 circle-image" style="background:url('http://www.wallpaperjosh.xyz/wp-content/uploads/2016/02/Cute-Girl-Josh071.jpg')">
                                </div>
                                <div class="col-md-7 col-xs-7 chat_text_padding">
                                    <h5 class="sanserif_font remove_margin_bottom position_relative opacity_8">Ganny</h5>
                                    <h6 class="chat_inline_margin opacity_4">hi</h6>
                                </div>
                                <div class="col-md-2 col-xs-2 user_right_icon_padding">
                                    <i class="fa fa-star-o pull-right"></i>
                                </div>
                            </div>
                            <div class="col-md-12 col-xs-12 user_div_padding left-first-div" ng-class="{active: $index == selected}" ng-click="loadChatData(users,$index)" ng-repeat="users in favouriteSelectedUsers">
                                <div class="col-md-4 col-xs-4 circle-image" style="background:url([[users.user.profile_picture]])">
                                </div>
                                <div class="col-md-7 col-xs-7 chat_text_padding">
                                    <h5 class="sanserif_font remove_margin_bottom position_relative opacity_8">[[users.user.name]]</h5>
                                    <h6 class="chat_inline_margin opacity_4">[[users.user.last_msg]]</h6>
                                </div>
                                <div class="col-md-2 col-xs-2 user_right_icon_padding">
                                    <i class="fa fa-star-o pull-right"></i>
                                </div>
                    ...
                    <div class="col-md-12 col-xs-12" >
                        <div class="row">
                            <ul class="list-inline ul_padding_left">
                                ...
                                <li id="star" ng-click="loadFav()"><i class="fa fa-star"></i></li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 chat_user_main_col_padding user_main_col_height user_main_col_border_bottom">
                        ...
                        <div id = "chat-container"  style="position: relative;width: 100%;height: 89%;overflow-y: auto;overflow-x:hidden;top: 3px">
	                       ...
	                       <div class="ph-float">
					        <a href="" id="load_more" ng-click="loadMoreConversation(userChatDetail)" class='ph-button ph-btn-grey'>{{trans('websocket_chat.load_more')}}</a>
					    </div> 
                                ...
                                <ul class="chat_ul list-inline" >
								    ...
								    <li class="self" ng-if="message.recipient=='self'" id="msg-[[message.id]]">
								       ...
								       <div class="chat" >
							              ...
							              <div class="deletethismsgself" ng-if="desktop">
    ...
    <div class="modal-content user_block_modal_content encounterexceeds_modal">
      ...
      <div class="modal-body " style="color: black">
	      ...
	      <div style="position:relative">
            <video id="localVideo" autoplay muted style="width: 10%;position: absolute;bottom: 0px;"></video>
       ...
       <div>
        ...
        <input type="button" id="stop" value="Stop Calling"></input>
        </div>
        <div>
		 ...
		 // Taken from http://stackoverflow.com/a/105074/515584
            ...
            $("#call").on("click", function(){
                ...
                callingTimer = setInterval(function(){
                    ...
                    serverConnection.emit("calling", { "from_user" : '684', "to_user" : '676'  });
                    $("#call-status").text("you calling..");
            ...
            serverConnection.on("calling", function(data){
                
                //caller = data.from_user;
                //callee = data.to_user;
                $("#call-status").text("socket_id : " + data.from_user + " calling..");
            });
            ...
            $("#stop").on("click", function(data){
                ...
                serverConnection.emit("stopped_calling", { "from" : '676', "to" : '684' });
                $("#call-status").text("you stopped calling..");

            });

            serverConnection.on("stopped_calling", function(data){
                console.log(data);
                $("#call-status").text("socket_id : " + data.from + " stopped calling..");
            });



            $("#accept").on("click", function(){
                serverConnection.emit("call_accepted", {"from_user" : '684', "to_user" : '676' });
               initiateCall();
                callonprogress = true;
...
*/
            ...
            function start() {
                ...
                if(caller) {
                    peerConnection.createOffer().then(createDescriptionForCaller).catch(errorHandler);
                }
            }
             ...
             function stopRemoteStream(event) {
                   console.log(event);
                   if(event.type == "iceconnectionstatechange" && (event.target.iceConnectionState == 'disconnected' || event.target.iceConnectionState == 'closed')) {
                       localStream.getVideoTracks()[0].stop();
                       localStream.getAudioTracks()[0].stop();
                       localStream.stop();
                       $("#call-status").text("call stopped");
                ...
                if(signal.sdp) {
                    ...
                    peerConnection.setRemoteDescription(new RTCSessionDescription(signal.sdp)).then(function() {
                        ...
                        if(signal.sdp.type == 'offer') {
                            ...
                            peerConnection.createAnswer().then(createDescriptionForCallee).catch(errorHandler);
...
console.log('ice candidate got');
                    ...
                    if(caller)
                    {
	                    serverConnection.emit('message', {"to_user" : '684', "message": {'ice': event.candidate, 'uuid': uuid} } );
                    }
                    else{
	                    
	                    serverConnection.emit('message', {"to_user" : '676', "message": {'ice': event.candidate, 'uuid': uuid} } );
            ...
            function createDescriptionForCallee(description) {
                ...
                peerConnection.setLocalDescription(description).then(function() {
                    serverConnection.emit('message', {"to_user" : '676', "message" : {'sdp': peerConnection.localDescription, 'uuid': uuid} } );
                }).catch(errorHandler);
            }
            ...
            function createDescriptionForCaller(description) {
                ...
                peerConnection.setLocalDescription(description).then(function() {
                    serverConnection.emit('message',{"to_user" : '684', "message" : {'sdp': peerConnection.localDescription, 'uuid': uuid} } );
                }).catch(errorHandler);
            }
