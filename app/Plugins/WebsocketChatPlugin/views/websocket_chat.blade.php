...
<link rel="stylesheet" href="@theme_asset('css/emojione.min.css')">
...
@if(!Session::has('chat_disabled') && !session::get('chat_disabled')) 	
...
<!-- 	                            <i class="fa fa-video-camera chat_video_audio" data-toggle="modal" data-target="#myModalVideoAudio"></i> -->
                                ...
                                <ul class="dropdown-menu">
                                        ...
                                        <li><a href="#/" data-target="#myModalReportUser" data-toggle="modal" data-user-id="[[userChatDetail.user.id]]">{{trans('app.report')}}</a></li>
                            ...
                            <div class="row fade2" ng-repeat="message in chatUsers[selected].user.messages" id="post-[[ ::$index ]]" scroll-to-last="post-[[ ::$index ]]">
                                <ul class="chat_ul list-inline" >
									...
									<li class="other" ng-if="message.recipient=='other'" id="msg-[[message.id]]">
								       <div class="chat" >
							                 <div class="avatar" ng-if="desktop" ng-hide="message.type==10 || message.type==11 || message.type==12"><img ng-src="[[chatUsers[selected].user.profile_picture]]" draggable="false"/></div>
							                 
							                 <div class="avatar mobile_avatar" ng-if="mobile" ng-hide="message.type==10 || message.type==11 || message.type==12"><img ng-src="[[chatUsers[selected].user.profile_picture]]" draggable="false"/></div>

							                <div class="bubbleOther you msg" ng-hide="message.type==10 || message.type==11 || message.type==12">
							                    <p ng-if="message.type==0" ng-bind-html="message.text | emoji | videoembed" style="word-break: break-word;"></p>
								    ...
								    <li class="self" ng-if="message.recipient=='self'" id="msg-[[message.id]]">
								       ...
								       <div class="chat" >
							              ...
							              <div class="deletethismsgself" ng-if="desktop && message.type==0" style="word-break: break-word;">
...
@else
<div id="myModalChatDisabled" class="modal fade" role="dialog">
  <div class="modal-dialog" >
    ...
    <div class="modal-content user_block_modal_content encounterexceeds_modal">
      <div class="">
        ...
        <h4 class="report_photo_title">{{{session::get('chat_disabled')}}}</h4>
      </div>
      
      <div class="" style="text-align: center">
       
        
        <button type="button" class="btn btn-primary encounter_exceeds" data-dismiss="modal">{{{trans('app.close')}}} </button>
      </div>
    </div>
  ...
  <div class="modal-dialog" >
    ...
    <div class="modal-content user_block_modal_content encounterexceeds_modal">
      <div class="">
        ...
        <h4 class="report_photo_title" style="font-size: 18px;">{{trans('app.chat_limit_text_one')}} <span class="bold_hours">{{{$chat_limit_hours}}} {{trans('app.hours')}} </span>{{trans('app.chat_limit_text_two')}}</h4>
      </div>
      <div class="modal-body user_block_modal_body">
...
<script src="@plugin_asset('WebsocketChatPlugin/js/ng-file-upload.js') "></script>


<script src="@plugin_asset('WebsocketChatPlugin/js/jemoji.js') "></script>

<!-- load angular-moment -->
<script type="text/javascript" src="@theme_asset('js/angular-moment.min.js')"></script>


<script type="text/javascript" src="@theme_asset('js/match-media.min.js')"></script>


<script type="text/javascript" src="@theme_asset('js/emojione.min.js')"></script>
...
function loadChatUsers() {
    ...
    @if(Session::has('chat_disabled') && session::pull('chat_disabled'))
$("#myModalChatDisabled").modal("show");
