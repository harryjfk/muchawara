...
class EmailPlugin extends PluginAbstract
{
	...
	public function hooks()
	{
		...
		Plugin::add_hook('send_admin_email',function($data) {
            ...
            try {
                ...
                $email_setting = EmailPluginRepository::getEmailSettings($data->type);
                  ...
                  $data->subject = $email_setting->subject;
                    ...
                    $data->user2 = (property_exists($data, 'user2')) ? $data->user2 : $data->user;
					 ...
					 $subject = $data->subject;
					 $admins = EmailPluginRepository::getAllAdmins();
                    ...
                    if($email_setting->content_type == 1)
                    {
						...
						foreach($admins as $admin) {
				            ...
				            Mail::send('emails.'.$data->body, ['user' => $data->user, 'user2' => $data->user2, 'footer_text' => $footer_text], function ($message) use ($admin, $subject) {  
				                
				                $message->to($admin->username, $admin->name)->subject($subject);
				            });
				        }    
                    }
                    else
                    {
						...
						foreach($admins as $admin) {
				            Mail::send('emails.default', ['content' => $data->body], function ($message) use ($admin, $subject) {    
                ...
                Log::info('User failed to login.'.$e->getTraceAsString());
                return ['error' => $e];
            } 
