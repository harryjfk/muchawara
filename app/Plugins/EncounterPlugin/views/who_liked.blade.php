...
<div class="col-md-12 mid_body_container">
    <div class="row">
        ...
        <div class="col-md-12">
            @if($can_see_liked_me)
            @if($total_count_likedme!=0)
            @foreach($wholiked as $l)
            
             
                        <div ng-click="gotoprofile('{{{url("/profile/$l->id")}}}')" class="col-md-4 col-xs-12 person_box" style="
                ...
                @if(count($l->photos))
                <div class="photo-counter"> <i class="fa fa-camera user_photos_list"></i><span class="small">{{{count($l->photos)}}}</span> </div>
                @endif
                <ul class="list-inline">
                    @if($l->onlineStatus())
                    <li style="float:right"><i class="fa fa-circle small_circle" style="color:#00BF00;display:inline"></i></li>
                    @endif
            ...
            <div class="" style = "color : black;text-align: center">
                ...
                <div class="mv20 fs16" style="font-size: 18px;">
                    {{{trans_choice('app.who_liked_msg',3)}}} <span style="font-size: 25px">{{{$total_count_likedme}}}</span> {{{trans_choice('app.who_liked_msg', 2)}}}
                </div>
         ...
         <div class="pagination_cnt">{!! $wholiked->render() !!}</div>
    </div>
</div>
{{--@endif--}}
@endsection
