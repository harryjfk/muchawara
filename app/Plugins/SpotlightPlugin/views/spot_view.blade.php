...
<div class="container demo-3">
    <div class="main">
        ...
        <div class="col-md-2 margin-top-20">
            <h5 class="upper text-center" style="margin-top: 0px; margin-bottom: 0px;">
                ¿Deseas
            </h5>
            <h5 class="upper text-center" style="margin-top: 0px">
                verte aqui?
            </h5>
            <a class="btn btn-default btn-width-120" href="#"><span class="upper">COMPARTE</span></a>
        </div>

       <div class="col-md-10 margin-top-bottom-10">
        <ul id="carousel" class="elastislide-list">

        	{{--@if($spotlight_only_superpowers != 'true')--}}

        		{{--<li data-toggle="tooltip" title="{{{trans_choice('app.add_me_here', 0)}}}" id="add_me_here" ><a href="#" style="position: relative"><img   class="thumb"   src="data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src="{{{$auth_user->thumbnail_pic_url()}}}" data-description="Add Photo"><img src="data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src="{{{asset('images/solid_red_background_209961.jpg')}}}" class="transparentcover" /><i class="material-icons ico ico--white ico--plus-lg" data-toggle="modal"  data-target="#spot">add</i></a></li>--}}
            ...
            @foreach($spots as $spot)
                @if($credit->balance > 0)
                    <li><a href='{{{url("/profile/spotlight/".$spot["id"])}}}' data-toggle="tooltip" title="{{{$spot['name']}}}"><img  class="thumb spotlighthover" src="data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src="{{{asset('uploads/others/encounters/'.$spot['profile_pic_url'])}}}" alt="{{{$spot['name']}}}" /></a></li>
                @else
                    <li><a href='#' data-toggle="tooltip" title="Recarga las balas para poder hacer wara"><img  class="thumb spotlighthover" src="data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src="{{{asset('uploads/others/encounters/'.$spot['profile_pic_url'])}}}" alt="Recarga las balas" /></a></li>
                @endif
            @endforeach
        </ul>
       </div>
...
//    $( '#add_me_here' ).click(function(){
//
//    	$('.main').addClass('spotlight_index');
//
//    });
