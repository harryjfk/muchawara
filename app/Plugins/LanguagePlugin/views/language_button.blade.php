   ...
   $.getJSON(url, function( data ) {
		   ...
		   var data_modified= _.filter(data,function(val){
			   ...
			   return _.find(temp,function(item){
				   ...
				   if (val['ISO3166-1-Alpha-2'].toLowerCase()== item || val['ISO3166-1-Alpha-2'] ==item)
				   		return true;
		 ...
		 $('#options').flagStrap({
	      ...
	      selectedCountry: "{{{$selected_language}}}"=="zh-CN" ? "{{{$selected_language}}}":"{{{$selected_language}}}".toUpperCase()
	    });
	...
	.done(function(data){
      ...
      var lang = "";
      if($(this).data('val')== 'zh-CN')
       		lang =$(this).data('val');
      else
      	 	lang =$(this).data('val').toLowerCase();
