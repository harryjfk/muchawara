...
//        $preferTownship =   isset($auth_user->profile->prefer_township) &&
//             $auth_user->profile->prefer_township != ""  ? $auth_user->profile->prefer_township : $auth_user->township;
//
//        $preferCity =   isset($auth_user->profile->prefer_city) &&
//             $auth_user->profile->prefer_city != ""  ? $auth_user->profile->prefer_city : $auth_user->city;
//
//        $preferCountry = isset($auth_user->profile->prefer_country) &&
//             $auth_user->profile->prefer_country != "" ? $auth_user->profile->prefer_country : $auth_user->country;
...
//       dd($nearByUsers);
        ...
        $custom_filter_data->prefered_city =  $preferCity;
        $custom_filter_data->prefer_country = $preferCountry;
        $custom_filter_data->prefer_township = $preferTownship;
    ...
    public function searchFilter (Request $request) {   
        ...
        $prefer_country  = $request->prefer_country; //numeric, country Id
        $prefer_city     = $request->prefer_city; //numeric, city Id
        $prefer_township = $request->prefer_township; //numeric, township Id
...
//            && preg_match("/^[0-9]+$/", $prefer_distance))
    ...
    public function setPreferOnlineStatus (Request $req) {
        ...
        if ($prefer_online_status != 'all' && $prefer_online_status != 'online' && $prefer_online_status != 'new') {
            return response()->json(['status' => 'error']);
        }
    ...
    public function countryCityFilter(Request $request)
    {
        ...
        $townships = Township::where('name','LIKE',"%{$search}%")->get();
        ...
        {
            $resp[] = array('id' => sprintf('%s, %s', $city->name, $city->country->name),'text' => sprintf('%s, %s', $city->name, $city->country->name));
        }
        foreach ($townships as $township)
        {
            $resp[] = array('id' => sprintf('%s, %s, %s', $township->name, $township->city->name, $township->city->country->name),'text' => sprintf('%s, %s, %s', $township->name, $township->city->name, $township->city->country->name));
        }
