...
/*
	...
	*/
		...
		public function getNearbyPeoples ($log_user_id, $gender, $ageLower, $ageUpper, $distance = 1, $preferCity, $preferCountry, $preferTownship, $riseup_superpower = true) {
...
//		);
		...
		if($log_user->profile->prefer_online_status == "new") {
		...
		} else  if ($peoplenearby_only_superpowers == 'true') {
    ...
    public static function getFilteredUsers($unit, $gender, $dob_range, $blockedIds, $online_status, $city, $country, $township) {
        ...
        if($country !== null && $country !== "") {
            $users = User::whereBetween('dob', $dob_range)
                ...
                if(isset($country) && $country != "")
                 $users->where('user.country', '=', $country);
                if(isset($city) && $city != "")
                 $users->where('user.city', '=', $city);
                if(isset($township) && $township != "")
                 $users->where('user.township', '=', $township);
        } else {
            $users = User::whereBetween('dob', $dob_range)
                ->whereNotIn('user.id', $blockedIds)
                ->where('activate_user', '<>', 'deactivated');
        ...
        $users->join('profile', 'user.id', '=', 'profile.userid');
    ...
    public static function getUsersByPreferenceLocation($country, $city, $township) {

       return User::join('profile', 'user.id', '=', 'profile.userid')
            ->where(function ($query) use ($country, $city, $township) {
                $query->where (function ($query) use ($country, $city, $township) {
                    $query->where('user.country', '=', $country);
                    if(isset($city) && $city != "")
                        $query->where('user.city', '=', $city);
                    if(isset($township) && $township != "")
                        $query->where('user.township', '=', $township);
                });
//                    ->orWhere (function ($query) use($country, $city) {
//                        $query->where('profile.prefer_country', '=', $country)
//                            ->where('profile.prefer_city', '=', $city);
//                    });
            })
            ->orderBy('profile.popularity', 'DESC');

//         return User::where('country', '=', $country)
//                      ->where('city', '=', $city);
    }
...
//		$user->profile->prefer_distance_nearby = $filter['prefer_distance'];
        $user->profile->prefer_country = $filter['prefer_country'];
        $user->profile->prefer_city = $filter['prefer_city'];
        $user->profile->prefer_township = $filter['prefer_township'];
...
*/	
    ...
    */
    ...
    public function timeDiffMinutes($date1, $date2)
    {
    ...
    public function isBoosted($userID)
    {
    	$riseup = RiseUp::where('userid', $userID)->first();
    	 
    	if(!$riseup) {
    		return false;
    	} else if($this->timeDiffMinutes($riseup->updated_at, date('Y-m-d H:i:s')) > 30) {
    		return false;
    	} else {
    		return true;
    	}
