...
class BadWordFilter
{
    ...
    {
        $this->defaults = include Plugin::path('ContentModerationPlugin/repositories/bad_word_filter_config.php');
    ...
    public function getDirtyWordsFromString($string)
    {
        ...
        foreach ($this->badWords[0] as $word) {

            if (is_array($word)) {
                $word1 = $word[0];
                $fullword = ('all_pattern' == $word[1]) ? false : true;
            ...
            $word = preg_quote($word1);

            if (preg_match($this->buildRegex($word, $fullword), $string, $matchedString)) {
     ...
     * @return string
     */
    private function buildRegex($word, $fullWords = false)
    {
        ...
        $censorCheck =  $fullWords ? '/\b'.str_ireplace(array_keys($leet_replace),array_values($leet_replace), $word).'\b/i'
                                    :'/'.str_ireplace(array_keys($leet_replace),array_values($leet_replace), $word).'/i';
        return $censorCheck;
        ...
        if (!$this->badWords) {
            ...
            if (!$this->isUsingCustomDefinedWordList()) {
                ...
                switch ($this->config['strictness']) {
                    ...
                    case 'misspellings':
                    case 'all':
                        $this->badWords = $this->getBadWordsByKey(['all']);
                        break;
