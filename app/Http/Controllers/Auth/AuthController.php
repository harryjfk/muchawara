    ...
    */
    ...
    public function postRegister (Request $request) {
        ...
        try {
            ...
            /************************** Create register for country and city ***********/

            $rep = $this->registerRepo->setCountry($arr['country']);

//             response()->json(['status' => $rep]);

            $this->registerRepo->setCity($request->city, $request->country);
    ...
    public function landingpage (Request $request) {
        ...
        return Theme::view('auth.landingpage');
    }
        ...
        if (Auth::attempt( array('username' => $request->username, 'password' => $request->password), $remember_me)) {
            ...
            if ($activate_user == 'activated') {
                ...
                if ($this->userRepo->checkUserDataIncomplete($user))
                    return redirect()->intended(UtilityRepository::get_setting('default_after_login_route'))->with('data_incomplete', true);
                else
                    return redirect()->intended(UtilityRepository::get_setting('default_after_login_route'));

            } else {

                Auth::logout();
                
                if($user->activate_token == "ban") {
	                
	                session(['message' => trans('admin.account_ban_by_admin')]);
