...
class ProfileController extends Controller
{
	...
	public function redirectToSlugURL($id)
	{
        ...
        //Agregado por Adriel para restar 1 bala porque viene del spotlight y redireccionarlo al perfil
	public function redirectToProfileFromSpotlight($id)
	{
           $authUser = Auth::user();
           $slugName = $this->profileRepo->getSlugnameByUserID($id);
           
           if($authUser->credits->balance > 0){
               $authUser->credits->balance = $authUser->credits->balance - 1;
               $authUser->credits->save();
               return ($slugName) ? redirect(url("user/$slugName")) : response()->view('errors.errors_404',[], 404);
           }
	...
	public function showProfile ($slug_name) {
		...
		$auth_user = Auth::user();
			...
			return response()->view('errors.errors_404',[], 404);
                ...
                $visiting_user->profile->popularity = $popularity;
                $visiting_user->profile->save(); 
