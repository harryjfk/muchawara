...
class ReportAbuseController extends Controller {
	...
	public function reportUser (Request $req) {
		...
		if ($this->abuseRepo->validate_user_id ($auth_user->id, $user_id)) {
			...
			/*$fromEmail ='contacto@inmobile-cuba.com';
            Mail::raw('Se ha hecho un nuevo reporte a un usuario', function($messege) use($fromEmail){
                $messege->to('buzon.inmobile@gmail.com');
                $messege->from('contacto@inmobile-cuba.com','WARA');
                $messege->subject('Reporte de usuario');
            });*/
			...
			$this->abuseRepo->reportUserAbuse($auth_user->id, $user_id, $reason);
                        ...
                        $ofChatController->sendWaraMessage(User::findOrFail($user_id), "Un usuario te ha reportado por la siguiente razón: ".$reason);
