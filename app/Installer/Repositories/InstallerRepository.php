...
class InstallerRepository
{
	...
	public function startInstallation(&$errors)
	{
		...
		try {
			...
			$errors["error_type"] = "UNKNOWN_ERROR";
			$errors["status"]     = "unknown error";
			$errors["errors"]     = $e->getMessage()." file : ".$e->getFile()." line: ".$e->getLine();
			return $this;
		}
	}
	...
	public function setOtherDefaultSettings()
	{
		...
		$this->settings->set('default_custom_male', 'male.jpg');
		$this->settings->set('default_female', 'female.jpg');
		$this->settings->set('default_custom_female', 'female.jpg');
        ...
        $this->settings->set('debug_mode', 'true');
        $this->settings->set('default_after_login_route', 'encounter');
		...
		$arr['unit']        = '';
        $field = $this->profileManageRepo->post_add_field($arr);
        $field->code = 'gender';
        $field->save();
        ...
        $option = $this->profileManageRepo->post_add_field_option($option_arr);
        $option->code = 'male';
        $option->save();
	...
	public function initPaymentsTable()
	{
		...
		$payment->type = 'stored';
		$payment->heading_code = 'superpower_heading';
		$payment->sub_heading_code = 'superpower_subheading';
		...
		$payment->name = 'credit';
		$payment->type = 'stored';
		$payment->heading_code = 'credit_heading';
		$payment->sub_heading_code = 'credit_subheading';
	...
	public function createAdminTable()
   	{
  		...
  		Schema::create('admin', function (Blueprint $table) {
	            ...
	            $table->string('admin_purpose', 128)->nullable();
 ...
 //   		}
	...
	public function createOneTimePasswordsTable()
	{
		Schema::dropIfExists('one_time_passwords');
  		...
  		Schema::create('one_time_passwords', function (Blueprint $table) {
	            ...
	            $table->string('otp_type', 128); 
	            $table->string('otp_code', 128); 
	            $table->timestamps();	            
	            $table->softDeletes();   
        });
