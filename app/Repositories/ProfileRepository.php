...
use App\Models\osAppVersionHistories;
use App\Models\User;
...
class ProfileRepository
{
	...
	public function __construct(FieldOptions $field_options, UserFields $user_fields, Fields $fields, UserPreferences $user_preferences, Photo $photo, FieldSections $field_sections, FacebookFriends $facebook_friends, User $user, Encounter $encounter, Visitor $visitor, Interests $interests, UserInterests $user_interests, OsAppVersionHistories $osAppVersion)
	{
		...
		$this->osAppVersion = $osAppVersion;
	}
	...
	public function getFullCityByUserId($userID)
	{
            $user = $this->user->where('id', $userID)->first();
            $country = $user->country;
            $city = ($user->city != '') ? ', '.$user->city : '';
            $town = ($user->township != '') ? ', '.$user->township : '';
            //var_dump($country,$city, $town);die;
            //$ciudad = $country.''. (!is_null($city)) ? ' ,'.$city : ''.(!is_null($town)) ? ' ,'.$town : '';
	...
	public function calculate_popularity($id,$max_count)
	{
        ...
        /*
         * Hecho por adriel para calcular la popularidad de acuerdo al porciento de personas que le han dado like
         * sobre el total de personas que han visitado el perfil
         */ 
	public function calculate_popularity_wara($id)
	{
		//$count = $this->visitor->where('user2','=',$id)->count(); // cantidad total de visitantes
                $dislikes = $this->encounter->select('likes')->where('user2','=',$id)->where('likes', '=', 0)->count();
                $likes = $this->encounter->select('likes')->where('user2','=',$id)->where('likes', '=', 1)->count();
                $count = $dislikes + $likes;
                
                if($count == 0)
			$popularity = 0;
		else
			$popularity = ($likes/$count)*100;
		return $popularity;
	}
	...
	public function save_resize_photo ($file, $fileName) {
		...
		$thumbnail      = $this->resize_photo($file, 70, 70, true);
	...
	public function findInterest ($interest) {
		...
		$searchString = (strlen($interest) > 100) ? substr($interest, 0, 100) : $interest;
		$interestObject = $this->interests->where('interest','=', $searchString)->first();
		return $interestObject;
		/*

		if(strlen($interest) > 100) {
			$searchString = substr($interest, 0, 100);
			$inter = $this->interests->where('interest','LIKE', $searchString.'%')->first();
		} else {
	...
	public function isUserInterestExist($userID, $interest) {

		$interest = $this->findInterest($interest);
		return ($interest && ($userInterest = $this->getUserInterestByInterestID($userID, $interest->id))) ? true : false;
	}


	public function getUserInterestByInterestID($userID, $interestID) 
	{
	...
	*/
    ...
    public function getCityCountryFromLatLong($lat, $long)
    {
    	$json = file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?latlng=".$lat.",".$long."&key=".$this->settings->get('google_map_api_key'));
    ...
    public function setOsAppVersionUser($user, $os, $versionApp) {
        $osAppVersion = $this->osAppVersion->where('user_id','=', $user->id )->get()->first();

        if(is_null($osAppVersion)) {
            $entity = new OsAppVersionHistories();
            $entity->user_id = $user->id;
            $entity->os = $os;
            $entity->app_version = $versionApp;

            $entity->save();
        } else {
            $osAppVersion->os = $os;
            $osAppVersion->app_version = $versionApp;
