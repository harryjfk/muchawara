...
class CreditRepository implements PaymentInterface
{
	...
	public function gateway_packages($metadata) {
		...
		foreach($packages as $package) {
			...
			$obj->name = trans($package->name_code).' '.$package->amount." ". UtilityRepository::get_setting('currency') .' : '.$package->credits.' '.trans('app.credits');
			$obj->id = $package->id;
    ...
    public static function takeBullet($user) {
        /*if($user->take_credits) {
            $user->take_credits = false;
            $user->save();

            $user->credits->balance = $user->credits->balance +
                                      UtilityRepository::get_setting('add_bullets_by_commands');
