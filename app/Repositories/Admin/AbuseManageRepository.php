...
class AbuseManageRepository {
	...
	public function getAllUnseenUserReports() {
		...
		$unseens = $this->usreAbuseReport->where('action', '=', 'unseen')->orderBy('created_at', 'asc')->get();
    ...
    public function removePhotoReport ($id) {
    	...
    	if ($report) {
    		...
    		if (!isset($report->withTrashed()->photos->deleted_at)) {
                ...
                $report->reportinguser->credits->balance = $report->reportinguser->credits->balance + 5;
                $report->reportinguser->credits->save();
    		
    	}
    }
    
    public function reportSuccess ($id) {

    	$report = $this->usreAbuseReport->find($id[0]);
    ...
    public function doPhotoAbuseTask($task, $data) {
        ...
        if (count($datas) > 0) {
            ...
            switch ($task) {
                ...
                case 'markseen':
                    $this->setMarkSeenPhotoReport($datas);
