...
class AdminServiceProvider extends ServiceProvider {
    ...
    public function boot()
    {
       ...
       /* admin master layout compose menu list multable */
       view()->composer('admin.layouts.admin', function($view){

            $guest = $this->isGuestAdmin();
            $view->with('guest', $guest);
            if($guest) 
                $view->with(
                    'guest_admin_routes', 
                    app('App\Repositories\Admin\AdminManagementRepository')->accessibleRoutesByAdminID(
                        session('admin_id')
                   )
                );
       });
    ...
    {
        return (session()->has('role') && session('role') === 'guest')? true : false;
    }
