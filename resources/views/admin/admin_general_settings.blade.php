...
<section class="content">
    <div class="col-md-12 section-first-col user-section-first-col">
        <form action = "#" method = "POST" id = "set-title-form">
            ...
            <div class="row">
                <div class="col-md-10 add-creditpackage-col add-interest-div">
                    <p class="add-credit-package-text">Mensaje de difusión</p>
                    <div class="form-group">
                        <input type="text" value = "@if($difusion){{{$difusion}}}@endif" placeholder="Mensaje de difusion" id = "difusion" name = "difusion" class="form-control  input-border-custom">
                    </div>
                    <button type="button" id = "set-msg-difusion" class="btn btn-info btn-addpackage btn-custom btn-set">{{trans_choice('admin.set', 1)}}</button>
                </div>
            </div>
            <div class="row">
                <div class="col-md-10 add-creditpackage-col add-interest-div">
                    <p class="add-credit-package-text">{{trans_choice('admin.msg_text', 1)}}</p>
                    <div class="form-group">
                        <input type="text" value = "@if($msg_text){{{$msg_text}}}@endif" placeholder="{{trans_choice('admin.msg_text', 1)}} {{trans_choice('admin.text', 1)}}" id = "msg" name = "msg" class="form-control  input-border-custom">
                    </div>
                    <button type="button" id = "set-msg-text" class="btn btn-info btn-addpackage btn-custom btn-set">{{trans_choice('admin.set', 1)}}</button>
                </div>
            </div>
            <div class="row">
                <div class="col-md-10 add-creditpackage-col add-interest-div">
                    <p class="add-credit-package-text">{{trans_choice('admin.url_update_app', 1)}}</p>
                    <div class="form-group">
                        <input type="text" value = "@if($url_update_app){{{$url_update_app}}}@endif" placeholder="{{trans_choice('admin.url_update_app', 1)}} {{trans_choice('admin.text', 1)}}" id = "upload" name = "upload" class="form-control  input-border-custom">
                    </div>
                    <button type="button" id = "set-upload-url" class="btn btn-info btn-addpackage btn-custom btn-set">{{trans_choice('admin.set', 1)}}</button>
                </div>
            </div>
        </form>
            
        <form action = "{{{ url('/admin/generalSettings/default-after-login-route') }}}" method = "POST">
        {!! csrf_field() !!}
        <div class="row">
            <div class="col-md-10 add-creditpackage-col add-interest-div">
                <p class="add-credit-package-text">{{trans('admin.default_page_after_login')}}</p>
                <div class="form-group">
                    <select name = "route" class="form-control input-border-custom select-custom">
                        <option>---</option>
                        @foreach($after_login_routes as $route)
                            <option value = "{{$route['route']}}" @if($default_after_login_route==$route['route']) selected @endif>{{$route['route_text']}}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="btn btn-info btn-addpackage btn-custom btn-set">{{trans_choice('admin.save', 0)}}</button>
                </div>
            </div>
        </div>
        </form>
        ...
        <form method = "POST" id="set-domain-form">
            {!! csrf_field() !!}
            <div class="row">
                <div class="col-md-10 add-creditpackage-col add-interest-div">
                    <p class="add-credit-package-text">{{trans('admin.domain_name_title')}}</p>
                    <div class="form-group">
                    ...
                    <button type="button" class="btn btn-info btn-addpackage btn-custom btn-set" id = "set-domain">{{trans_choice('admin.save', 0)}}</button>
                </div>
            </div>
        </form>
            ...
            <div class="col-md-10 add-creditpackage-col add-interest-div">
                ...
                <p class="add-credit-package-text">{{{trans("admin.set_default_prefer_genders")}}}</p>
                    ...
                    @foreach($gender->field_options as $option)
                        ...
                        <label class="package-label" style="margin-top:2px; margin-right:10px;">{{trans('admin.for')}} {{{trans('custom_profile.'.$option->code)}}}</label>

                        <span class="prefer_gender_inner_box">
                        @foreach($gender->field_options as $gend)
                            
                                <label class="package-label" style="margin-top:2px; margin-right:10px;">{{{trans('custom_profile.'.$gend->code)}}}</label>
                                <label class="switch">
                                    <input class="switch-input prefer-gender" type="checkbox" data-gender="{{$option->code}}" data-prefer-gender='{{$gend->code}}'
                                    @if(isset($prefered_genders[$option->code]) && in_array($gend->code, $prefered_genders[$option->code])) checked @endif/>
                                    <span class="switch-label"></span> 
                                    <span class="switch-handle"></span>
                                </label>
...
<script>
    ...
    if("{{session()->has('default_page_after_login')}}" == "true") {
        toastr.success("{{trans_choice('set_status_message',0)}}");
    }




    function addPreferedGender(gender, prefer_gender)
    {
        var URL = "{{url('admin/generalsettings/save-prefer-genders')}}";
        var data = {
            gender : gender,
    ...
    function removePreferedGender(gender, prefer_gender)
    {
        var URL = "{{url('admin/generalsettings/save-prefer-genders')}}";
        var data = {
            gender : gender,
    ...
    $(".prefer-gender").on('change', function(){

        var gender = $(this).data('gender');
        var prefer_gender = $(this).data('prefer-gender');
        
        if($(this).is(':checked')) {
            addPreferedGender(gender, prefer_gender);
        } else {
            removePreferedGender(gender, prefer_gender);
        }
    ...
    $("#set-domain").on("click", function(){

        var URL = "{{url('admin/generalSettings/domain/save')}}";
        var formData = $("#set-domain-form").serializeArray();
    ...
    $("input[name=secure_mode]").on("change", function(){

        var URL = "{{{ url('admin/generalSettings/enable-disable-secure-mode') }}}";
        ...
        $.post(URL, formData, function(res) {
            ...
            if(res.status == "success") {
                ...
                $("input[name=secure_mode]").prop('checked', false);
            }
...
<script>
    ...
    $('#set-msg-text').click(function(){
      ...
      $.post("{{{ url('/admin/generalSettings/msg_text') }}}", data, function(response){
    ...
    $('#set-msg-difusion').click(function(){
    
      var data = {};
      data.title = $('#difusion').val();
    
      data._token="{{{csrf_token()}}}";
    
      $.post("{{{ url('/admin/generalSettings/msg_difusion') }}}", data, function(response){
    ...
    $('#set-upload-url').click(function(){
    
      var data = {};
      data.title = $('#upload').val();
    
      data._token="{{{csrf_token()}}}";
    
      $.post("{{{ url('/admin/generalSettings/upload_url') }}}", data, function(response){
