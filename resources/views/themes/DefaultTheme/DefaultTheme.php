...
//			$pre_content = '<li class=""><a href="{{url}}"><i class="{{class}}">{{symname}}</i>{{title}}</a><span class="red span_red_margin notification-{{notif_type}}">{{notif_count}}</span></li>';
            ...
            $pre_content = '<li class=""><a href="{{url}}"><img src="{{url_img}}" class="{{class}}"/><span class="text-sidebar border-text-menu">{{title}}<span></span></a><span class="red span_red_margin notification-{{notif_type}}">{{notif_count}}</span></li>';
			...
			foreach ($data as $link) {
				...
				$temp_content = str_replace("{{url_img}}", (isset($link['url_img'])) ? $link['url_img'] : '', $temp_content);
...
SCRIPT;
		...
		Theme::render_modifier('header-menus', function($data){

			$content = "";

			foreach($data as $array) {
				$active = \Request::is($array['route']) ? 'active' : '';
				$content .= "<li  class=\"{$active}\">
								<a href=\"{$array['link']}\">{$array['text']}</a>
							</li>";
			}
