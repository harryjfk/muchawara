...
<div class="col-md-3 pad right_box_master">
	...
	{{--@else--}}
		...
		{{--</div> --}}
  {{--@endif--}}
		<div class="your-prof margin-top-75">
			
			@if($percent != '100')
			<div class="complete">
				
				<h2 class="complete-your-profile">{{{ trans("app.profile_status")}}}<h2>
			...
			<div class="complete">
				...
				<h3 style="display: none">{{{ trans("app.complete_profile")}}} {{{$percent}}}% {{{ trans("app.complete")}}}.</h3>
				 ...
				 <p style="width:{{{$percent}}}%"><span class="profile_header_rightbar">{{{$percent}}}%</span></p>
				<progress class="css3" max="100" value="{{{$percent}}}"></progress>
	...
	@if($percent != '100')
	    <a class="margin-left-25" href="{{{url('profile/'.$auth_user->id)}}}"><button type="button" class="btn btn-default custom_modal-popup edit_profile"><span class="">{{{ trans("app.complete")}}}</span></button></a>
    @else
		<a class="margin-left-25" href="{{{url('/encounter')}}}"><button type="button" class="btn btn-default custom_modal-popup edit_profile"><span class="">{{{trans('app.play_encounters')}}}</span></button></a>
	@endif

	<div class="likes-box">
		<p class="text-center margin-bottom-10">Gustos mas populares</p>
		<div class="scroll-left">
			<div style="display: inline-block; margin-left: 10px;">
		    	<input type="checkbox">
				<label>Boquisucio</label>
			</div>
			<div class="clear" style="display: block; margin-left: 10px;">
				<input type="checkbox">
				<label>Boquisucio</label>
			</div>
			...
			<a class="margin-left-25" href="#"><button type="button" class="btn btn-default custom_modal-popup edit_profile"><span class="">{{{trans('app.upload')}}}</span></button></a>
