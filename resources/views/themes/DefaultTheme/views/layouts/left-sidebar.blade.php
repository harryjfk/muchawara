                ...
                <ul class="special_features">
                    ...
                    <li>
						<div style="position: relative">
							<a href="{{{url('profile/'.$auth_user->id)}}}"><img class="img-circle" src="{{{ $auth_user->thumbnail_pic_url()}}}" alt="...">
								<span class="text-color-position">	{{{$auth_user->name}}} </span>
								@if($hide_popularity !='true')
									@if($auth_user->profile->popularity < 25)
										<span data-toggle="tooltip" class="popular position-popularity" title="{{{trans_choice('app.popular_values',1)}}} {{{$auth_user->profile->popularity}}}"><img class="margin-right-0" src="{{{asset('images/1_pop_bajo.png')}}}" width="60px" height="30px" /></span>
									@elseif($auth_user->profile->popularity >= 25 && $auth_user->profile->popularity < 50)
										<span data-toggle="tooltip" class="popular position-popularity"  title="{{{trans_choice('app.popular_values',2)}}} {{{$auth_user->profile->popularity}}}"><img class="margin-right-0" src="{{{asset('images/2_pop_media.png')}}}" width="60px" height="30px" /></span>
									@elseif($auth_user->profile->popularity >= 50 && $auth_user->profile->popularity < 75)
										<span data-toggle="tooltip" class="popular position-popularity" title="{{{trans_choice('app.popular_values',3)}}} {{{$auth_user->profile->popularity}}}"><img class="margin-right-0" src="{{{asset('images/3_pop_alta.png')}}}" width="60px" height="30px" /></span>
									@else
										<span data-toggle="tooltip" class="popular position-popularity" title="{{{trans_choice('app.popular_values',4)}}} {{{$auth_user->profile->popularity}}}"><img class="margin-right-0" src="{{{asset('images/4_pop_mango.png')}}}" width="60px" height="30px" /></span>
									@endif
								@endif
							</a>
						</div>
...
--> 
				   ...
				   <li class="divider-left-picture-profile"></li>

                     <li>
						 <div>
							 <!-- 							<i class="fa fa-remove"></i> -->
							 <img class="img-bullets" src="@theme_asset('images/bullets.png')" height="30" width="23"/>
							 <span style="vertical-align: middle;" data-toggle="tooltip" class="user_score" title="{{{ trans('app.credits')}}} {{{$credit->balance}}}">{{{$credit->balance}}}</span>
						 </div>
                     </li>
