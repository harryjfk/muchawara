    ...
    $(document).ready(function(){
      ...
      //km
        ...
        if(non_superpower_range_enabled == 'true')
        {
	        if(superpower_user)
	        {
    ...
    for (var i = 0; i < oldSeerchfields.length; i++) {
      ...
      if(field.on_search_type=='dropdown'){
       ...
       $(this).parent().find('.dropdowncustom').html(optionshtml);
        ...
        show: "bounce",
        hide: "explode",
        checkAllText: check_all_text, //Check all
        uncheckAllText: uncheck_all_text, //Uncheck all
        noneSelectedText: select_options_text, //Select options
        selectedText: selected_text, //# selected
    ...
    $('#contry-city-select-filter').select2({
        // dropdownParent: $('#filterModal'),
        language: 'en',
        ajax: {
            url: contryCityFilter_url,
            dataType: 'json',
            delay: 300,
            ...
            processResults: function (data, params) {
                // var results = [];
                // for(var ind in data) {
                //     var obj = {id: data[ind].id, text: data[ind].text};
                //     results.push(obj);
                // }
    ...
    $('.savefiltersettings').on('click',function(){
        ...
        var contryCityTownship = $('#contry-city-select-filter').val();

        var splitValues = contryCityTownship.split(",", 3);

        if(splitValues.length == 1 )
        {
            customfields['prefer_country'] = splitValues[0];
        } else if(splitValues.length == 2) {
            customfields['prefer_country'] = splitValues[1];
            customfields['prefer_city'] = splitValues[0];
        } else if(splitValues.length == 3) {

            customfields['prefer_township'] = splitValues[0];
            customfields['prefer_country'] = splitValues[2];
            customfields['prefer_city'] = splitValues[1];
        }
